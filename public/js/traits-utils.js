(function(window){
  ...
  async function renderTraits(){
    ...
    const sequence = ++traitRenderSequence;
    if (!dom.traits.children.length) {
      dom.traits.innerHTML = '<div class="trait-extra">Beräknar…</div>';
    }
    const data = storeHelper.getTraits(store);
    const KEYS = ['Diskret','Kvick','Listig','Stark','Tr\u00e4ffs\u00e4ker','Vaksam','Viljestark','\u00d6vertygande'];
    const currentChar = (store.characters || []).find(c => c.id === store.current) || null;
    if (dom.cName) {
      dom.cName.textContent = currentChar?.name || '';
    }
